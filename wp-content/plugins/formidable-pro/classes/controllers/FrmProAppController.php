...
class FrmProAppController {
	...
	private static function get_version_string_for_combined_js_file() {
		...
		if ( $lite_version !== $pro_version ) {
			$version = $pro_version . '-' . $lite_version;
		} else {
			// If the versions both match, we can use a single version.
			$version = $pro_version;
		}

		$version .= FrmProAppHelper::use_jquery_datepicker() ? '-jquery' : '-flatpickr';
		return $version;
	}
