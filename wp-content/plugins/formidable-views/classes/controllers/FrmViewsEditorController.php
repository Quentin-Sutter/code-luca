...
class FrmViewsEditorController {
	...
	public static function view_editor() {
		...
		$is_timeline_type         = FrmViewsDisplaysHelper::is_timeline_type( $view );
	...
	private static function render_metaboxes( $view ) {
		...
		if ( ! isset( $wp_meta_boxes['frm_display'] ) ) {
			return;
		}
	...
	private static function register_view_editor_scripts_and_styles() {
		...
		try {
			...
			// Catch and ignore errors as well.
			// The Bluehost plugin throws an error when admin_enqueue_scripts is called twice.
		}
	...
	public static function route_ajax() {
		...
		switch ( $action ) {
			...
			case 'frm_views_get_calendar_view_options':
				$function = 'get_view_field_options';
				break;

			case 'frm_views_get_timeline_view_options':
				$function = 'get_view_field_options';
				break;
	...
	private static function get_data() {
		...
		$is_timeline_type = FrmViewsDisplaysHelper::is_timeline_type( self::$view );
		...
		} elseif ( FrmViewsDisplaysHelper::is_timeline_type( self::$view ) ) {
			$response_data = array_merge(
				$response_data,
				array_merge(
					FrmViewsTimelineEditorHelper::get_js_options_values( self::get_option_value( 'timeline_options' ) ),
					array(
						'fieldOptions' => self::get_field_options(),
					)
				)
	...
	private static function get_view_field_options() {
	...
	private static function update_view() {
		...
		} elseif ( $is_timeline_type ) {
			$options['timeline_options'] = FrmViewsTimelineEditorHelper::get_settings_from_post_request();
		}
	...
	private static function pull_form_info() {
		...
		$response_data = array(
			...
			'timelineDateFormats'        => FrmViewsTimelineEditorHelper::get_date_formats(),
		);
		wp_send_json_success( $response_data );
	}
	...
	/**
	 * Echo the default timeline styles.
	 *
	 * @since 5.8
	 * @return void
	 */
	private static function echo_default_timeline_styles() {
		echo wp_json_encode( FrmViewsTimelineEditorHelper::get_default_styles() );
	}
