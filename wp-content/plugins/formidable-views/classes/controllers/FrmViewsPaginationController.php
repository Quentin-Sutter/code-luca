...
class FrmViewsPaginationController {
	...
	private static function maybe_offset_modals() {
		...
		if ( ! class_exists( 'frmBtsModApp' ) ) {
			// This is only required when the Bootstrap modal add-on is active.
			return;
		}
		...
		/**
		 * Set a maximum of 1000 modals by default to prevent abuse.
		 *
		 * @since 5.8
		 *
		 * @param int $max_modal_offset
		 */
		$max_modal_offset = (int) apply_filters( 'frm_max_modal_offset', 1000 );
		$modal_offset     = min( $modal_offset, $max_modal_offset );
	...
	public static function add_ajax_pagination_scripts() {
		wp_register_script( 'frm_views_pagination', FrmViewsAppHelper::plugin_url() . '/js/pagination' . FrmViewsAppHelper::js_suffix() . '.js', array( 'wp-hooks' ), FrmViewsAppHelper::plugin_version(), true );
		$js_vars = array(
