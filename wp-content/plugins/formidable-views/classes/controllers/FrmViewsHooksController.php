...
class FrmViewsHooksController {
	...
	public static function load_hooks() {
		...
		add_filter( 'frm_usage_snapshot', 'FrmViewsAppController::add_usage_tracking_data' );

		if ( ! is_callable( array( 'FrmUsrDirController', 'letter_filter' ) ) ) {
			add_shortcode( 'frm-letter-filter', 'FrmViewsAppController::letter_filter' );
		}
	}
	...
	public static function load_admin_hooks() {
		...
		add_action( 'restrict_manage_posts', 'FrmViewsDisplaysController::before_filter_button' );
		...
		// Add the Views Gutenberg block assets
		if ( is_callable( 'register_block_type' ) ) {
			add_action( 'enqueue_block_editor_assets', 'FrmViewsSimpleBlocksController::block_editor_assets' );
		}
	}
	...
	public static function load_ajax_hooks() {
		...
		// Handle date format via AJAX.
		add_action( 'wp_ajax_frm_views_date_i18n', 'FrmViewsDisplaysController::ajax_get_date_format' );
		add_action( 'wp_ajax_nopriv_frm_views_date_i18n', 'FrmViewsDisplaysController::ajax_get_date_format' );
	...
	private static function load_editor_ajax_hooks() {
		...
		add_action( 'wp_ajax_frm_views_get_map_view_options', $editor_route );
		add_action( 'wp_ajax_frm_views_get_calendar_view_options', $editor_route );
		add_action( 'wp_ajax_frm_views_get_timeline_view_options', $editor_route );
