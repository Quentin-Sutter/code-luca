...
<script>
	...
	frmViewsEditorInfo = {
		...
		addressFieldOptions: <?php self::echo_address_field_options( $form_id ); ?>,
		statusFieldId: <?php echo absint( self::get_status_field_id() ); ?>,
		isTableType: <?php echo absint( $is_table_type ); ?>,
		isMapType: <?php echo absint( $is_map_type ); ?>,
		isTimelineType: <?php echo absint( $is_timeline_type ); ?>,
		previewLimit: <?php echo absint( FrmViewsAppHelper::get_visual_views_preview_limit() ); ?>,
		timelineDateFormats: <?php echo wp_json_encode( FrmViewsTimelineEditorHelper::get_date_formats() ); ?>,
		timelineThemeColors: <?php echo wp_json_encode( FrmViewsTimelineEditorHelper::get_themes_colors() ); ?>,
		hasListingContent: '<?php echo esc_js( empty( $view->post_content ) ? 0 : 1 ); ?>'
	};
	...
	<?php if ( $is_timeline_type ) { ?>
		frmViewsEditorInfo.defaultTimelineStyles = <?php self::echo_default_timeline_styles(); ?>;
	<?php } ?>
</script>
