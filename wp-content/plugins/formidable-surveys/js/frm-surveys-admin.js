	...
	const FrmSurveys = {
		...
		handleRadioOptions: function() {
			...
			const updateBulkEditVisibility = ( target, displayFormat ) => {
				...
				fieldSettings.querySelector( '.frm-ai-generate-options-modal-trigger' )?.classList.toggle( 'frm-force-hidden', shoudHideBulkEdit );
			};
			...
			const onChangeUseImagesInButton = useImagesInButtonCheckbox => {
				const fieldSettings = useImagesInButtonCheckbox.closest( '.frm-single-settings' );
				const isChecked = useImagesInButtonCheckbox.checked;

				fieldSettings.querySelector( '.frm-bulk-edit-link' )?.classList.toggle( 'frm_hidden', isChecked );
				fieldSettings.querySelector( '.frm-ai-generate-options-modal-trigger' )?.classList.toggle( 'frm-force-hidden', isChecked );
			};
		...
		handleLikertOptions: function() {
			...
			const onClickAddOption = event => {
				...
				wrapperEl.querySelectorAll( '.frm_likert_delete_opt.frm_disabled' )
				...
				event.target.closest( '.frm_likert_opt' ).after( newOpt );
			...
			const onClickDeleteOption = event => {
				...
				const visibleOptions = wrapperEl.querySelectorAll( 'li:not(.frm_hidden)' );
				if ( visibleOptions.length === 2 ) {
					Array.from( visibleOptions )
						.find( li => li !== optionEl )
						.querySelector( '.frm_likert_delete_opt' )?.classList.add( 'frm_disabled' );
				}
		...
		handleBulkEditLink: function() {
			...
			const onClickBulkEdit = event => {
				...
				const bulkEditButton = event.target.tagName.toLowerCase() === 'a' ? event.target : event.target.closest( 'a' );

				const fieldId = bulkEditButton.closest( '[data-likert-id]' ).getAttribute( 'data-likert-id' );
				const opts    = bulkEditButton.closest( '.frm_likert_opts_container' ).querySelectorAll( '.frm_likert_opt input' );
				...
				bulkOptionType.value = bulkEditButton.getAttribute( 'data-option-type' );
			...
			document.addEventListener( 'click', event => {
				if ( event.target.matches( '.frm_likert_bulk_edit_opts' ) || event.target.closest( '.frm_likert_bulk_edit_opts' ) ) {
